...
> → **`RELEASE HOLD`는 2026-09-02에 해제됐다** (아래 표 마지막 행).
>
> | | HOLD 사유 |
> |---|---|
> | **2026-08-18 (아래 기록)** | 운영/linked runtime read-only confirmation, **운영 dry-run**, **Release A~D 승인**, browser 2~8세션 Realtime |
> | **2026-08-29** | ~~**W9 미해결 4건**~~ → **조사 후 2건** — `wiki-snapshot` 429 대량 재발(그룹), 관전 이모티콘 미전달. ~~RETIRE 사유 불일치~~ 수정 완료·**미배포**, ~~`username-lookup` 404~~ **결함 아님으로 종결** |
> | **2026-09-02 (현재)** | **없음 — HOLD 해제.** 남은 2건이 닫혔다: 429는 감축 배포 후 재스모크에서 **502 0건**(W8-b·W9-b), 이모티콘은 **스펙 범위 밖으로 종결**. **W10(유지보수 게이트 해제)이 수행됐고 서비스가 열려 있다.** 단일 기준은 `docs/agent/CURRENT.md` §1 |
>
> 위 표 왼쪽 항목이 어떻게 닫혔는지:
>
> | 미체크 항목 | 2026-08-28 창 |
