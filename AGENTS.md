...
- **유지보수 게이트(`VITE_MAINTENANCE=true`)가 사용자 노출을 막는 유일한 방패다.**
  ...
  | 4 | RETIRE 사유 불일치 | 결과 화면에서 로비로 나갈 수 없다 — **수정 완료(2026-08-29)이나 미배포이므로 운영 번들에는 그대로 있다** |
  | ~~5~~ | ~~`username-lookup` 404~~ | **결함 아님 — 종결 (2026-08-29).** 아이디 미존재 시의 의도된 응답 |
  | 6 | 관전 이모티콘 미전달 | — **(스펙이 수신자를 규정하지 않아 결함이 아닐 가능성. 재검증 필요)** |

  목록과 상태는 `docs/agent/CURRENT.md` §5.5가 단일 기준이다.
  **2026-08-29 기준 실제 미해결은 3·6이며, 4는 배포 대기다. 그래도 금지는 그대로다** —
  게이트를 끈 채 push하면 3번과 미배포 상태의 4번이 즉시 노출된다.
