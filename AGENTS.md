...
> **③ 아래에서 push가 한 번 실행됐다 — 2026-09-03** `[사용자 실행]`.
> `git push origin feat/group-final-gaps:main` (`9eba7e9..a784d2e`, 17커밋: 코드 3 + merge 3 +
> 문서 11). 트랙 A·B·N2를 배포했고 **DB·Edge Function 변경은 0이다.**
> **③이 요구하는 승인 조건이 충족된 건별 승인이었다** — "이 변경을 실사용자에게 지금 노출해도
> 되는가"에 대한 판단 + 배포 전 검증(`npm test` 204/204 · `npm run build` exit 0 · 로컬 스모크).
> 기록: `docs/agent/CURRENT.md` §3의 2026-09-03 상자와 `main` push 이력 표 **#3**.
>
> **이 선례도 다음 push까지 확장되지 않는다.** 승인은 그 push에서 끝났다 —
> **"게이트가 없어도 저번에 올렸으니까"는 승인이 아니다.** §1의 문장과 같은 규칙이며,
> ③은 약해지지 않았다. **③ 아래의 push는 매번 그 시점의 변경 내용으로 다시 판단한다.**
...
- **2026-09-03의 push(#3)도 선례로 인용하지 않는다.** 그것은 **③ 아래의 첫 사례**이며
  (위 상자), 성립한 이유는 게이트가 아니라 **그 변경에 대한 건별 판단과 배포 전 검증**이다.
  **판단 대상은 매번 다르므로 결과도 매번 다시 정한다** — `main` push는 계속 건별 승인이다.
- **문서 전용 커밋은 `main`에 올리지 않는다.** 배포에 아무 영향이 없으면서 프로덕션 재배포만
  발생시킨다. 백업은 `feat` 브랜치로 충분하다. **2026-09-03 배포로 두 ref가 잠시 같아졌지만**
  (`main` = `feat` = `a784d2e`) **다음 문서 커밋부터 다시 갈라지는 것이 정상이다**
  (`docs/agent/CURRENT.md` §3). 이번 push에 문서 11건이 함께 올라간 것은
  **코드 배포에 딸려 간 것**이며, 문서만으로 `main`을 건드린 것이 아니다.
...
**예외 — 트랙 세션(worktree)은 `CURRENT.md`를 수정하지 않는다.** 트랙은 자기 작업만 커밋하고,
**통합 세션이 merge 시점에 한 번에 반영한다.**

> **왜.** `CURRENT.md`는 저장소 전체의 SSOT다. 트랙 둘이 각자의 worktree에서 동시에 이 파일을
> 고치면 **merge마다 SSOT 충돌이 난다** — 내용이 서로 맞더라도 같은 줄을 양쪽이 건드리기 때문에
> 기계적으로 충돌하고, 해소 과정에서 한쪽의 사실이 조용히 사라질 수 있다. 트랙이 손대지 않으면
> **교집합이 0으로 유지되고**, 통합 세션은 merge된 실제 상태를 보고 한 번에 정확히 적는다.
>
> 트랙 세션에도 위의 "기준 커밋이 뒤처지면 먼저 보고한다"는 그대로 적용된다. **읽고 보고하되
> 고치지 않는다.** 반영할 내용은 통합 세션에 인계한다.
