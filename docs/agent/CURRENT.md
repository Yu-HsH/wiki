...
측정 시점: **2026-08-27, 로컬 HEAD `77094d1`.** 아래 값은 그 시점 실측이며 재측정 명령을 함께 적는다.

| 항목 | 값 | 재측정 명령 |
|---|---|---|
| `origin/main` HEAD | `e6d8eee` ("0529백업") — **변경 없음** | `git ls-remote origin refs/heads/main` |
| `origin/feat/group-final-gaps` HEAD | `77094d1` (`git ls-remote` 실측 2026-08-27). 이 파일의 갱신 커밋은 같은 push에 포함되므로 **push 후 값은 그 커밋이다** | `git ls-remote origin refs/heads/feat/group-final-gaps` |
...
| `origin/main...HEAD` | 0 behind / **22 ahead** (`77094d1` 기준, 2026-08-27 실측. 이 파일의 갱신 커밋을 더하면 23) | `git rev-list --left-right --count origin/main...HEAD` |
...
- 로컬 22개 커밋에는 미배포 서버 권위 V2와 Packet 13이 들어 있고, **운영 DB에는 해당 RPC가 없다**
...
- **cutover 계획은 작성 완료** (`docs/ops/CUTOVER-PLAN.md`). 2026-08-27에 **§6.0 롤백 판단 기준(P11)**과
  **§6.3 확정 복원 절차·§6.5 리허설 기록**이 추가됐다. 남은 것은 창 밖 선행 조건 6건과
  창 실행 승인이다 (§5.2).
...
### 5.2 남은 작업 — CUTOVER-PLAN §7의 미완 선행 조건 6건

전부 창 **밖에서** 끝낸다. 하나라도 미완이면 창을 열지 않는다.
**운영·외부 확인 4건(P4·P5·P7·P9), 측정 선행 + 범위 합의 1건(P10), 파일 준비 1건(P12)이다.**
P12만 저장소 안에서 처리할 수 있다.
...
| P9 | CLI 버전 확인 | 로컬 | `npx supabase --version` == `2.114.0` |
| P10 | **차단 행 측정 + 삭제 범위 확정** | 운영 측정 → 사용자 합의 | **성격이 2026-08-27에 바뀌었다.** 먼저 §5.3-0의 `w6_blocking_rows`를 측정한다 — **0보다 크면 삭제를 건너뛰는 선택지가 없다** (W6가 #10에서 `SQLSTATE 23514`로 실패, §6.5.3 리허설 근거). 그 다음 §5.3 (1)(2) 측정치를 보고 **삭제 범위**를 승인한다. 합의 대상은 "삭제할지"가 아니라 "어디까지"다 |
| P12 | 창 기록 파일 준비 | 저장소 | `docs/ops/CUTOVER-LOG-YYYY-MM-DD.md` 틀 (`AGENTS.md` §6) |

완료된 선행 조건: **P1·P2·P3**(유지보수 게이트 구현·포함·로컬 확인), **P6**(`backup/` gitignore),
**P8**(link 대상이 운영인지 확인), **P13**(커밋 기준 `npm test`·`npm run build` 재실행)
— 2026-08-21 확인. **P11**(롤백 판단 기준) — 2026-08-27 확인, CUTOVER-PLAN §6.0.
...
| `docs/ops/CUTOVER-PLAN.md` | **운영 cutover 실행 계획.** W0~W11 창 절차, 백업·삭제·롤백 전문, 창 전 선행 조건(§7), 창 후 검증(§8), 미결정 항목(§9), Release A~D 대체 매핑(§10). **창을 열기 전 §7을 이 문서 기준으로 점검한다** |
