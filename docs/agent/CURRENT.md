...
갱신 날짜: 2026-09-02
기준 커밋: `cc47fe6` (`docs: record measured RLS policy counts and validate the derivation method`)
마지막 **코드** 커밋: `0ad3cde` — 그 뒤는 전부 문서다. 판정의 코드 축은 여기서 움직이지 않았다
이전 기준: `a32d362` · `48e3f2d`
...
>
> **지금 답이 필요한 것 다섯:** G1(reward 테이블 소유권) · G3(3코스 제약 교체 창) ·
> G11(reason 어휘 통일) · G2(아이템 Realtime 경로) · 시안 판정 대기 3건(G16·G18·G19).
...
>
> **창이 확정적으로 필요한 첫 항목은 `17a-1 오늘 3코스`다.** `daily_challenges`의
> **`UNIQUE(challenge_date)` 제약 교체**가 필요하고 이는 운영 DDL이다 —
> `PACKET-CONTRACT-GAPS.md` §5. **그리고 15가 그것을 기다린다.**
...
| **`docs/design/MOBILE-VALIDATION-CORRECTIONS.md`** | **모바일 시안 정정 기록 (2026-09-02 신설).** 시안이 "확정값의 변환, 새 기능 없음"을 선언했으나 어긋난 지점을 남긴다 — **판정 완료 3건**(순차 해제·XP 수치·칭호 = 전부 시안 오류)과 **판정 대기 3건**. **시안 HTML은 수정하지 않았다** |
