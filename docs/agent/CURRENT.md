...
갱신 날짜: 2026-08-27
기준 커밋: `2031143` (`docs: clear P9, P10 and P12, leaving only the time-bound preconditions`)
...
| **창 전 선행 조건 (P1~P14)** | **12/14 완료.** 남은 것은 **P4·P14 2건.** P4는 창 당일 확인이고, **P14(복원 도구 `psql` 확보)는 2026-08-27 신설** — 이 머신에 `psql`이 없어 §6.3 복원을 실행할 수 없다 | CUTOVER-PLAN §7·§6.3.0. 목록은 §5.2 |
...
측정 시점: **2026-08-27, 로컬 HEAD `2031143`.** 아래 값은 그 시점 실측이며 재측정 명령을 함께 적는다.

| 항목 | 값 | 재측정 명령 |
|---|---|---|
| `origin/main` HEAD | `e6d8eee` ("0529백업") — **변경 없음** | `git ls-remote origin refs/heads/main` |
| `origin/feat/group-final-gaps` HEAD | 직전 push로 `c6f8db6` (`git ls-remote` 실측 2026-08-27). 이 파일의 갱신 커밋은 같은 push에 포함되므로 **push 후 값은 그 커밋이다** | `git ls-remote origin refs/heads/feat/group-final-gaps` |
...
| `origin/main...HEAD` | 0 behind / **26 ahead** (`2031143` 기준, 2026-08-27 실측. 이 파일의 갱신 커밋을 더하면 27) | `git rev-list --left-right --count origin/main...HEAD` |
...
- 로컬 26개 커밋에는 미배포 서버 권위 V2와 Packet 13이 들어 있고, **운영 DB에는 해당 RPC가 없다**
...
### 5.2 남은 작업 — CUTOVER-PLAN §7의 미완 선행 조건 2건

전부 창 **밖에서** 끝낸다. 하나라도 미완이면 창을 열지 않는다.

| ID | 항목 | 성격 | 비고 |
|---|---|---|---|
| P4 | **프로젝트 Active 확인** | 외부 (Supabase 대시보드) | **창 당일에 다시 확인한다.** 무료 요금제는 7일 무활동 시 자동 일시정지하고 최종 플레이가 2026-08-04이므로 전날 값이 당일을 보장하지 않는다. **창을 막는다** |
| **P14** | **복원 도구(`psql`) 확보** | 로컬 환경 | **2026-08-27 신설.** 이 머신에 `psql`·`pg_dump`·`pg_restore`가 **없다** `[산출물]`. §6.3 복원은 41KB 스키마 덤프와 수백 KB 데이터 덤프를 복원해야 해서 Studio SQL Editor로 대체할 수 없다. **창은 막지 않지만 롤백을 막는다** — 해소 경로는 CUTOVER-PLAN §6.3.0, (a) 로컬 스택 컨테이너의 `psql`(설치 불필요) 권장 |

**완료 12건.** 2026-08-21: **P1·P2·P3**(게이트 구현·포함·로컬 확인), **P6**(`backup/` gitignore),
**P8**(link 대상 확인), **P13**(커밋 기준 `npm test`·`npm run build`).
2026-08-27: **P5·P7·P9·P10·P11·P12.**
...
| `docs/ops/wipe-public.sql` | **§6.3 2단계 실행 파일.** `public`·`private` 스키마를 비우고 덤프가 담지 않는 스키마 속성 2건(owner, PUBLIC USAGE)을 복원한다. **파일 안에 자체 `begin;`/`commit;`이 있어 바깥에서 감싸도 롤백되지 않는다 — 시험 실행이라는 것이 없다** (§6.3.1-0) |
