...
마지막 **코드** 커밋: `527f896` — **2026-09-03에 코드 축이 다시 움직였다.** 트랙 A(`e70c541`)와
트랙 B(`7a7197e`)가 `feat/group-final-gaps`에 `--no-ff`로 통합됐다 (`b3da192` · `527f896`).
`0ad3cde` 이후 처음 들어온 코드 변경이며 **아직 배포되지 않았다** (§3)
이전 기준: `b281e01` · `e1b5546` · `f40e071`
브랜치: `feat/group-final-gaps`

> # ⚑ 2026-09-03 — **트랙 A·B가 통합됐다. 배포되지 않은 코드가 브랜치에 있다.**
>
...
> | 항목 | 실측 |
> |---|---|
> | **`npm test`** | **203/203 pass, fail 0** — 144(베이스라인) + 17(A) + 42(B). **단순 합과 일치한다** |
> | **`npm run build`** | **exit 0** |
> | **grep 불변식** | **A 7건 + B 8건 전건 통과** (측정 단서 5건은 `TRACKS.md` §8-A·§8-B 완료 행) |
> | **DB** | **미접근. migration 산출물 0개** — 두 트랙 모두 프론트만 만졌다 |
>
> **⚠ 운영에는 아직 없다.** `527f896`은 `feat/group-final-gaps`에 있고 `main`에 없다.
...
- **트랙 A·B — 코드 완료, 배포 전 (2026-09-03).** `feat/group-final-gaps`에 `b3da192`·`527f896`으로
  통합됐다. **`npm test` 203/203 · `npm run build` exit 0 · migration 0 · 운영 DB 미접근.**
  **진행 중인 축은 코드가 아니라 배포다** — 게이트가 해제된 상태라 `main` push는 실사용자에게
  바로 닿는다 (§3, `AGENTS.md` §1.1). **배포 승인은 아직 없다.**
  상세는 §5.0의 2026-09-03 블록과 `TRACKS.md` §8-A·§8-B 완료 행.
...
> | **D** 15a XP 원장 | **착수 가능 — 순차 자원만 비면 된다** | — | 파일 충돌 0(전부 신규). **A·B가 끝났으므로 로컬 Supabase 스택이 비었다** — `TRACKS.md` §1.1의 순차 사유가 해소됐다 |
>
> **다음 트랙은 D(15a)다.** C는 G7(아이템 ID 확정)이 사람 결정이라 코드로 풀 수 없고,
> **D는 선행 0 · 파일 충돌 0 · 자원 경합 0**이 됐다. 티켓은 `TRACKS.md` §8-D다.
>
...
> ### ▶ **신규 결함 2건 (2026-09-03, 트랙 B가 찾았다)** `[코드]`
>
> | # | 결함 | 상태 |
> |---|---|---|
...
> | **N2** | **`MainPage.jsx:413`(분기점 `ad569f2` 기준 `:402`) 랭킹 카드의 "전체 보기 →"가 게스트에게 열려 있다** — `/ranking`은 `ProtectedRoute`인데(`App.jsx:202-207`) 게이팅이 없어 **누르면 `/login`으로 튕긴다.** 같은 형태의 결함을 B가 **온라인 플레이 카드**(`:230-249`)에서는 닫았다 | **미수정.** **랭킹 탭 블록이라 `TRACKS.md` §8-B 범위 밖 ⑦**이다. **소유는 랭킹 화면 트랙**이며 `TRACKS.md` §5.2에 **부채-2**로 등재했다. 고치는 방법은 B가 이미 쓴 `disabled` + `aria-disabled` 형태 그대로다 |
>
> **둘 다 "게스트가 로그인 전용 경로를 만났을 때"의 같은 결함군이다.** N1은 **거짓말을 했고**,
> N2는 **막다른 길로 보낸다.** 패킷 17 §6의 게스트 경계는 **N2가 닫혀야 완결된다.**
