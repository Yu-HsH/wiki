...
>
> **3코스 창에 5항목을 묶었다** — 17a-1 · C4 CHECK · C3-① · G2-② · 15b.
> **대상 테이블 4개가 겹치지 않아 DDL 충돌은 없다.** 순서 의존 2건:
> **C3-① → 15b**(컬럼 단위 grant를 먼저 좁히면 `total_xp`에 권한이 붙지 않는다) ·
> **15a → 15b**.
>
> **⚠ 게이트 재판정: `G2-②`(`room_events` INSERT 회수) 하나가 판정을 뒤집는다.**
> 배포된 프론트가 **10곳에서 직접 INSERT한다**(`MultiplayerGamePage.jsx:191-204`) `[코드]`.
> 회수하면 1:1 아이템 모드가 즉시 깨지고, 프론트를 배포해도 **구버전 번들 drain 문제**가
> 남는다 — **8월 창에서 게이트가 담당했던 그 문제다.**
> **G2-②를 빼면 나머지 4항목은 게이트 없이 가능한 성격을 유지한다** `[추정]`.
> **빼는 것을 권한다** (`TRACKS.md` §7.3·§7.4).
>
> **⚠ 계약 문서 불일치 3건이 드러났다** (`TRACKS.md` §9. 계약은 이 세션에서 고치지 않았다):
> ① **C5의 적용 지점은 4곳도 5곳도 아니라 8곳**이고 이름 fallback은 **6종**이다
> (`나`·`상대`가 빠졌다) ② **C4의 "CHECK 2건"은 추가 1건**이다 — §4.2가
> `match_end_reason`을 이미 제외했다 ③ **C3-①의 grant 목록에 `updated_at`이 빠졌다** —
> **문서 그대로 적용하면 닉네임·프로필 사진 저장이 권한 오류로 깨진다**
> (`ProfilePage.jsx:86·149`가 둘 다 `updated_at`을 함께 보낸다) `[코드]`.
> **③이 가장 위험하다 — 보안 수정이 사용자 기능을 깨는 형태다.**
