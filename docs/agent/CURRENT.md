...
**2026-08-28에 저장소 전체를 훑어 "W6 이전 상태를 현재 사실로 단정하는 서술"을 조사했다** `[산출물]`.
아래가 그 결과다. **`AGENTS.md`는 처리했고 나머지 5개 문서가 남았다.**

| # | 항목 | 상태 | 왜 |
|---|---|---|---|
| 2 | ~~**`AGENTS.md` §1.1 근거 서술 갱신**~~ | **완료 (2026-08-28).** | 금지 근거를 "운영에 V2 RPC가 없다"에서 **"유지보수 게이트가 유일한 방패이고 W9 미해결 4건이 그 뒤에 있다"** 로 교체했다. §1에 "창의 승인은 그 창에서 끝났다"를, 기준 문서 목록에 `PROD-SNAPSHOT`·`CLAUDE_HANDOFF` 낡음 경고를 함께 넣었다 |
| **1** | **새 운영 스냅샷 `PROD-SNAPSHOT-YYYY-MM-DD.md` 작성** | 미수행 | `PROD-SNAPSHOT-2026-08-20.md`가 **무효다.** 함수 7→36, RLS 12/14→14/14, 이력 0→12행. 기존 파일은 **보존**한다. 운영 재조회가 필요하므로 별도 작업 |
| **5** | **`docs/CLAUDE_HANDOFF.md` 갱신 — 우선순위 1위** | 미수행 | **낡은 서술을 가장 많이, 아무 표시 없이 담고 있고 `AGENTS.md`가 모든 세션을 여기로 안내한다.** §3.1 제목이 "운영 미적용", `:146` "미적용 (2026-08-20 실측 확인)", `:147` "함수 7개뿐 / V2 RPC 30개는 운영에 없다", `:185` RELEASE HOLD 남은 게이트 4개(전부 완료됨), `:250` "publication·RLS·Edge Function 배포 목록 미확인"(W7·W8이 해소), `:278` "schema_migrations 부재 → cutover 차단 요소"(W3이 해소), `:280` "`finish_group_player`는 현재 운영에만 존재"(W6이 삭제), `:281` 3개 migration 미적용(적용됨), `:291` "Release A~D 재설계 전에는 운영 적용을 시작하지 않는다". **임시 조치로 `AGENTS.md` 기준 문서 목록에 경고를 달아 뒀다** |
| 6 | **`docs/WIKI_RACE_GROUP_DB_SECURITY_SPEC.md` 갱신** | 미수행 | 이 파일은 이미 `stale`로 표기돼 있지만 **그 표기는 시간 규칙(15분/3분)에 대한 것뿐이다.** 운영 상태 서술은 덮이지 않았다: `:6`·`:175` "운영 적용 상태: 미적용", `:348` "다음 테이블은 **현재** RLS 최종 잠금이 필요하다"(W7에서 `rls_off_tables = 0`), `:1041~1043` "`db push`/`migration repair` 금지"(둘 다 승인 아래 실행됨) |
| 3 | **`qa/30-INTEGRATION-CHECKLIST.md` §21 갱신** | 미수행 | `:300`의 미완 체크박스가 "운영 dry-run, Release A~D 승인"을 남은 항목으로 두고 `:302`가 그것을 근거로 `RELEASE HOLD`를 판정한다 — **HOLD 판정은 유지되지만 사유가 바뀌었다** (지금은 W9 4건이다). CUTOVER-PLAN §10.2 |
| 7 | `wiki-race-2.0-handoff/code/` 4개 문서 | 미수행 (낮음) | `10-CODE-MASTER-TODO.md:29-30`(§2 작업 순서 — **날짜 없는 현재 상태표**라 가장 오해하기 쉽다), `:215`·`11-REPOSITORY-AUDIT.md:207`·`13-GROUP-FINAL-GAPS.md:206`(날짜 있는 판정 기록이라 형식상은 이력이지만 다른 문서가 "현재 판정"으로 인용한다), `18-...md:39`·`:84`("현재 작업에서는 실행하지 않는다" + Release B/C/D 절차 — **U2 대체 표기가 이 파일 안에는 없다**) |
| 4 | `target-level` Edge Function 존재 확인 | 미수행 | §8.1-7. `--prune` 미사용은 확인됐으나 실물 확인은 안 했다 |
