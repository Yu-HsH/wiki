...
> `0ad3cde`(`wiki-snapshot` 요청 감축, W9 발견 3). **2026-09-02 W1-b로 `main`에 올렸다** —
> 프론트는 배포됐고 **Edge Function은 아직이다** (§3의 "미완료").
...
2026-08-27~28 창이 **DB·배포 축을 전부 닫았고**, 남은 HOLD 사유는 ~~W9에서 발견된 결함 4건~~ → **발견 3 하나**뿐이다 (2026-09-02).
...
**2026-08-29 조사로 4건 중 2건이 닫혔고, 2026-09-02 W1-b 배포로 발견 4가 운영에서도 해소됐다.**
**게이트 해제를 막는 것은 이제 발견 3 하나다** — 코드는 고쳤으나 **Edge Function이 미배포라
운영 요청 수는 여전히 61건**이다. 조사 근거는 §5.5, 배포 상태는 §3.

| # | 항목 | 상태 | 게이트 해제 차단? |
|---|---|---|---|
| **3** | **`wiki-snapshot` 502 대량 재발 (4인 그룹).** 준비 버튼 11회 연속 실패. **62요청 × 참가자 수**로 곱해지는 구조 | **감축 적용 완료 (2026-09-02). 프론트만 배포(W1-b), Edge Function 미배포 · 운영 미검증.** 대기실 4인 **244 → 124건**, 게임 진입 **244 → 31건** `[산출물]`. **대기실은 절반만 줄었다** (§5.4-1·2) | **예 — Edge Function 배포 + 재스모크까지** |
| **4** | ~~**"유효하지 않은 RETIRE 사유"** — 결과 화면에서 로비 나가기 실패~~ | **해소 (2026-09-02 W1-b 배포).** 원인은 RPC 계약 불일치가 **아니라** `onClick` 바인딩 버그였다 `[코드]`. 프론트 3줄이라 Vercel 배포로 완결된다. §5.5-4 | **아니오** |
...
> **2026-09-02 갱신 — 위 판정은 그대로 유효하다. 근거만 좁아졌다.**
> 발견 4는 W1-b로 배포돼 운영에서 해소됐다. **그러나 발견 3이 남아 있고, 그것도 반쪽이다** —
> 프론트만 올라갔고 `supabase functions deploy wiki-snapshot`이 아직이라 **운영 요청 수는
> 변하지 않았다** (§3의 "미완료"). 게이트를 지금 해제하면 사용자는 **준비 버튼 실패**를
> 그대로 만난다. 선행 조건은 **Edge Function 배포 + 4인 재스모크 통과**다.
...
측정 시점: **2026-09-02, 로컬 HEAD `9eba7e9`** (`git ls-remote origin` 실측 `[산출물]`).
이전 측정: 2026-08-29 `e272b44` (두 ref가 5커밋 갈라져 있던 시점) / 2026-08-28 23:21 `4a78a0d`.
아래 값은 **`main`·`feat` 양쪽 push 후 실측**이며 재측정 명령을 함께 적는다.
...
| 항목 | 값 | 재측정 명령 |
|---|---|---|
| `origin/main` HEAD | ~~`4a78a0d`~~ → **`9eba7e9`** (2026-09-02 W1-b). `4a78a0d`에서 **9커밋 앞**, `e6d8eee`에서 45커밋 앞 | `git ls-remote origin refs/heads/main` |
| `origin/feat/group-final-gaps` HEAD | **`9eba7e9`** — `main`과 **다시 동일 값** | `git ls-remote origin refs/heads/feat/group-final-gaps` |
...
| `origin/main...HEAD` | ~~0 behind / 8 ahead~~ → **0 behind / 0 ahead** — 두 ref가 같은 커밋을 가리킨다 | `git rev-list --left-right --count origin/main...HEAD` |
| `e6d8eee..HEAD` 커밋 수 | **45** (`9eba7e9` 기준, 2026-09-02 실측) | `git rev-list --count e6d8eee..HEAD` |
...
> ### ⚠ 미완료 — **Edge Function 배포가 아직이다. 지금은 반쪽 상태다**
>
> `0ad3cde`는 배포 경로가 둘로 갈리는 첫 커밋이다.
>
> | | 배포 경로 | 상태 |
> |---|---|---|
> | 프론트 (`pages/`·`services/`) | `main` push → Vercel | **완료 (W1-b, 2026-09-02)** |
> | **Edge Function** | **`npx supabase functions deploy wiki-snapshot`** | **미실행** |
>
> **이 순서가 맞다.** 프론트가 먼저면 새 프론트가 `includeDocument`를 보내도 옛 함수는
...
>
> **따라서 지금 4인 스모크를 돌리면 발견 3은 그대로 재현된다** — 요청 수는 운영에서 아직
> 61건이다. **Edge Function 배포 전의 스모크 결과로 발견 3을 판정하지 않는다.**
...
**게이트 ON은 W1-b 실행 전 프로덕션 URL에서 점검 화면 렌더로 확인했다**
`[사용자 확인, 2026-08-29]`.
...
- **"미배포 RPC 호출" 위험은 해소됐다.** 운영 `public` 함수는 7개에서 **36개**가 됐고 legacy RPC
  2개는 삭제됐다 (W7). ~~2026-08-29부터 두 ref는 5커밋 갈라져 있다~~ → **W1-b(2026-09-02)로
  다시 같은 커밋이 됐다** (위 표). **DB 스키마를 건드리는 커밋은 그 사이에 하나도 없었으므로
  프론트/DB 버전 어긋남은 계속 없다.** 지금 어긋나 있는 축은 DB가 아니라
  **프론트 ↔ Edge Function**이다 — 위 "미완료" 상자.
...
1. **유지보수 게이트가 유일한 방패다.** ~~W9 미해결 4건(발견 3·4·5·6)~~ →
   ~~2026-08-29 조사 후 발견 3·6이 남았고 발견 4는 수정됐으나 미배포다~~ →
   **2026-09-02 W1-b로 발견 4는 배포됐다. 남은 것은 발견 3(운영 미검증)과 발견 6이다** (§5.5).
   사용자를 막고 있는 것은 `VITE_MAINTENANCE=true` 하나뿐이다. `VITE_*`는 **빌드 시점 인라인**이므로
   (F11) **게이트 값이 바뀐 상태에서의 push는 곧 서비스 오픈이다.**
2. **게이트 해제는 W10이고, W10은 별도 판단이다.** ~~발견 3·4~~ → **발견 3**이 해결되기 전의
   해제는 사용자가 **준비 버튼 실패**를 직접 만나는 결과가 된다.
   **그리고 발견 3은 아직 해결되지 않았다** — 코드 수정은 끝났지만 **Edge Function이
   미배포라 운영 요청 수는 여전히 61건**이고, 배포 후 4인 재스모크로 확인해야 한다 (§3의 "미완료").
...
- **cutover 창 — 실행 완료, W10 미수행으로 종료 (2026-08-27~28).**
  ...
  **2026-09-02 갱신 — ①이 끝났다.** 남은 순서는
  **~~① 프론트 배포(`main` push)~~ 완료(W1-b) → ② `npx supabase functions deploy wiki-snapshot`
  → ③ 4인 그룹 재스모크(대기실 124건이 통과하는가) → ④ 통과하면 발견 6 재검증,
  실패하면 §5.4-3 백오프 → ⑤ W10**이다. ①②를 뒤집으면 관전 화면이 깨진다 (§5.4-1·2).
...
- **W9 발견 3 `wiki-snapshot` 요청 감축 — 코드 수정 완료. 프론트만 배포 (2026-09-02 W1-b).**
  ...
  **배포 순서 제약이 있다 — 프론트를 Edge Function보다 먼저 올려야 한다** (§5.4-1·2의 경고 상자).
  **대기실은 절반만 줄었으므로 운영 재스모크 전에는 해결로 보지 않는다.** §5.4-3(백오프)은
  그 측정 뒤에 판단한다.
...
#### 5.4-3 429 백오프 — **다음 판단. 지금 넣지 않는다**

**순서:** ① 위 수정을 배포한다 → ② 4인 그룹 스모크를 다시 돌려 **대기실 준비 버튼이
124건으로 통과하는지** 본다 → ③ 그래도 429가 나면 그때 백오프를 넣는다.

**지금 넣지 않는 이유가 바뀌었다.** 이전 사유는 "62건이 남은 상태에서는 실행 예산을
태운다"였다. 32건이 된 지금 그 사유는 약해졌지만, **백오프는 실패를 느리게 만들 뿐
요청 수를 줄이지 않는다.** 감축만으로 통과하는지 먼저 재는 것이 순서다 —
통과하면 백오프는 불필요한 지연이 되고, 실패하면 그때는 "얼마나 모자라는가"라는
측정값을 들고 들어갈 수 있다.
...
| # | 항목 | 관측 | 원인 | 해제 차단 |
|---|---|---|---|---|
| **3** | **`wiki-snapshot` 502 대량 재발 (4인 그룹)** | 준비 버튼 **11회 연속 실패** | **구조 확정. 감축 적용 완료 (2026-09-02). 프론트만 배포(W1-b), Edge Function 미배포 · 운영 미검증.** 대기실 4인 244 → **124건**, 게임 진입 244 → **31건** (§5.4-1·2). **대기실은 절반만 줄었다** | **예 — Edge Function 배포 + 재스모크까지** |
| **4** | ~~**"유효하지 않은 RETIRE 사유"**~~ | 결과 화면에서 로비 나가기 실패 | **확정 → 수정 완료 (2026-08-29). 배포 완료 (2026-09-02 W1-b).** 아래 5.5-4 | **아니오** |
