...
| ~~**1**~~ | ~~**새 운영 스냅샷 `PROD-SNAPSHOT-YYYY-MM-DD.md` 작성**~~ | **완료 (2026-09-02) — `docs/ops/PROD-SNAPSHOT-2026-09-02.md`.** 08-20 대비 변화 4축(테이블 14→21, 함수 7→36, RLS off 2→0, 이력 0→12)을 **migration별로 매핑**했고, 새 테이블 7개·정책 0개 테이블 7개·publication 미변화의 근거를 `[코드]`로 붙였다. 기존 파일은 **보존**했다. **남은 공백은 그 문서 §9** — 테이블별 정책 수·함수 이름 목록·제약/ACL 대조가 이번 조회에 없었다 |
...
| **`docs/ops/PROD-SNAPSHOT-2026-09-02.md`** | **운영 상태의 현재 기준 (2026-09-02 실측).** 테이블 21 / 함수 36 / RLS off 0 / 정책 0개 테이블 7 / publication 4 / 이력 12행 / users 145 · `game_records` 59. **08-20 대비 변화를 migration별로 매핑**했고, 정책 0개가 결함이 아니라 3계층 접근 설계라는 근거(`[코드]`)와 **publication 미변화가 향후 실시간 기능에 거는 선행 조건**을 담았다. **이 문서가 담지 않은 축은 §0.2·§9** — 특히 **테이블별 정책 수는 미측정**이다 |
