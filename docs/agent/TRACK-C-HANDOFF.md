...
## 3.9 세 번째 스모크 — 통과. **트랙 C를 닫는다** `[사용자 확정, 2026-09-06]`

`f8c0eae` 상태에서 2세션 스모크를 다시 돌렸고 **통과했다.**

| 확인 | 결과 |
|---|---|
| 첫 입장 연출 | **VS 인트로 · 카운트다운 정상** — §3.7의 phase 정지가 닫혔다 |
| 지급 | **5칸** · `duel_item_events` **5건 전부 서버 확정** |
| 차단 판정 | **blind → `blocked`** — 편집 보호가 **서버에서** 판정됐다 |
| 일반 이동 | **5회 · `STATE_VERSION_CONFLICT` 0건** — §3.8 증상 1이 닫혔다 |
| 수용조건 ② | **`room_events` POST 0건** |

**차단이 서버에서 나온 것이 이 트랙의 목적이었다.** §3.6의 6번 항목이 그 이유로
만들어졌다 — 예전에는 서버가 적용으로 기록하고 피해자 화면만 방어로 보이는 어긋남이
가능했고, 그것을 클라이언트 면역 판정이 허용했다. 이제 두 화면이 같은 사건을 말한다.

`STATE_VERSION_CONFLICT` 0/5는 §3.8의 클라이언트 수정이 실효했다는 뜻이지만
**남은 1%를 반증하지는 않는다** — in-flight 하트비트 창은 5회로 드러날 확률이 아니다.
구조는 그대로 남아 있고 아래 §5에 관찰로 넘긴다.

### 관측된 400/404 3종 — **전부 C와 무관하다. 기록만 한다** `[사용자 판정, 2026-09-06]`

| # | 관측 | 원인 | 처리 |
|:-:|---|---|---|
| **1** | `ensure_today_daily_challenge` **400** | `db reset --local` 뒤 로컬 `daily_challenge_pool`이 **비어 있다 (count 0, 실측)**. 후보가 없으면 오늘의 도전을 만들 수 없다. **로컬 전용 상태이고 원격과 무관하다** | **3코스 창 대상.** 로컬은 `DAILY_POOL_INSERT.sql`로 채운다 |
| **2** | `username-lookup` **404** | **결함이 아니다.** 아이디 미존재 시의 의도된 응답이고 프론트가 이미 처리한다 (`authContext.jsx:44-46`) | **2026-08-29 종결.** `CURRENT.md` §5.5-5. 재개 대상 아님 |
| **3** | `set_duel_target_v2` **400** | Postgres 로그 실측 **`ERROR: DUEL_ROOM_NOT_WAITING` 1건.** 방이 이미 `waiting`을 벗어난 뒤 목표 제출이 도착한 것 — 늦은/중복 제출이다. 네 개의 `raise` 전부 클라이언트 상태 거절이고 경기 상태를 바꾸지 않는다 | **경기 진행 영향 없음.** C 범위 밖 |

**셋 다 아이템 경로를 지나가지 않는다.** 1은 로컬 시드 부재, 2는 종결된 정상 동작,
3은 대기실 제출 타이밍이다. 스모크 통과 판정을 막지 않는다.

### 트랙 C 완료

| | |
|---|---|
| **P1~P8** | 완료 (`ec64787` … `f0a4d86`) |
| **스모크 회귀 3건** | 완료 (`f8c0eae`) — phase 정지 · 버전 경합 · 도달 불가 링크 풀 |
| **최종 커밋** | `f8c0eae` · `feat/track-c` push 완료 |
| **게이트** | `npm test` **322/322** · build exit 0 · pgTAP **148/148** · preflight **11/11** · §2.3 불변식 7개 전수 유지 |
| **남기는 것** | §4(2건) · §5(통합 세션 반영 5건) |

**C가 만들지 않은 것은 §4에 그대로 있다** — G2-② 권한 회수는 프론트 배포 뒤 별도
창이고, C는 클라이언트 INSERT를 0건으로 만드는 것까지만 했다.
...
## 5. 통합 세션이 반영할 것 — 최종 5건 `[트랙 C 종료 시점, 2026-09-06]`

트랙 C는 닫혔다 (§3.9). 아래 다섯은 **C가 해결할 수 없거나 C의 산출이 아닌 것**이고,
각각 어디를 고쳐야 하는지까지 적는다. 앞 절들의 재서술이 아니라 **행동 목록**이다.

| # | 항목 | 성격 | 누가 반영하나 |
|:-:|---|---|---|
| **1** | §2.3 개수 불변식 — 5개가 6번 깨졌다 | **계약 문서 수정** | `TRACKS.md` §2.3 소유자 |
| **2** | 부채 ③ 강제 이동 목적지 커버리지 | **별도 웨이브** | 스냅샷 파이프라인 |
| **3** | `progress_version`이 OCC 토큰과 liveness 카운터를 겸한다 | **v2 권위 계약 변경** | 서버 권위 소유자 |
| **4** | 그룹 경로 `coalesce` 잠재 벽돌 | **관찰 — 조사 먼저** | 그룹 축 소유자 |
| **5** | `migration 무수정` 연속이 끊겼다 | **사실 기록** | 통합 세션 |

---

### 1. §2.3 개수 불변식 — **불변식 5개가 한 웨이브에서 6번 깨졌다**

§1이 분석이고 §1의 표가 사례다. 숫자를 정확히 읽어야 한다:
**깨진 불변식은 5개**(① · ④ · `duelSwapDisabled` 분기 · `navigate` 3곳 · ⑥)이고
**깨진 횟수는 6번**이다 — ④가 서로 다른 트랙에서 **두 번** 깨졌다. §1 소제목의
"다섯 번"은 불변식 수를 센 것이니, 표의 6행과 어긋난 것이 아니다.

**반영할 것:** ①③④를 개수에서 **assert로 교체**한다. 교체 문안은 §1에 있다.

- ① → `SINGLE_ITEM_IDS` 배열 일치 (현재 `grep -c`는 **4**이고 정상이다)
- ③ → `useItemSystem` 반환 키 13개를 배열로 (파일이 **트랙 B 소유**라 C는 손대지 않았다)
- ④ → "네 파일이 같은 키를 쓴다"를 assert (개수 6은 유지 가능하나 형태가 문제다)
- ⑤는 **그대로 둔다** — 줄머리 앵커라 이 웨이브에서 한 번도 깨지지 않았다. 모범 형태다.

**왜 통합 세션의 일인가:** 불변식은 `TRACKS.md`가 소유하고 C는 값을 지킬 의무만
있었다. C는 그것을 전수 유지했다 (§3.9 게이트). **고치라고 제안할 수는 있지만
고칠 권한은 없다.**

> ⚠ **위협받는 것은 이 문서가 아니라 코드 옆 주석이다.** ④의 grep은 코드 디렉터리를
> 훑으므로 이 문서는 범위 밖이고, 그래서 §1과 §2.3이 그 키를 따옴표째로 인용할 수
> 있었다 (실측: 이 문서에 3회, ④ 합계는 여전히 **6**). ⑤도 `css/multiplayer.css`
> 한 파일만 센다. **검열되는 것은 그 리터럴 옆에서 왜 그런지 적으려는 주석이고**,
> 회피가 두 번 일어난 곳이 정확히 거기다 — `tests/explorationRecords.test.js`가
> 일부러 인용을 피하고, 이 트랙이 인용했다가 7이 되어 되돌렸다. **불변식이 자기
> 설명을 검열한다**는 것이 교체 근거이고, 그 대상은 코드 주석이다.

### 2. 부채 ③ — 강제 이동 목적지 커버리지 (C안)

`20260904100000`이 후보 풀을 **해석 가능한 목적지로 좁혀** 실패를 미소비
(`NO_ELIGIBLE_LINK`)로 바꿨다. **아이템이 정직해졌을 뿐 좋아지지 않았다.**

실측 (§3.8): 스냅샷 10개 대 링크 목표 2,809개. 문서 10개 중 **3개는 후보 0**이고,
나머지도 1~3개다. 즉 강제 이동이 피해자를 **가 봤던 곳으로** 보내는 경향이 있다.

**반영할 것:** 이동 전에 후보 목적지를 미리 스냅샷하는 경로. 그러면 풀이 실물이 된다.

**주의 2건:**
- **가드를 푸는 방향으로 가지 말 것.** `20260904100000` 머리말이 왜 안 되는지
  적어 두었다 — `coalesce`가 `page_id`와 `revision_id`를 어긋나게 만들고
  이후 모든 링크 클릭이 `LINK_NOT_ALLOWED`가 된다. **한 번의 실패가 영구 벽돌이 된다.**
- **pgTAP는 커버리지가 올라가도 깨지지 않는다.** fixture의 해석 불가 링크(Page U ·
  Page M)는 남겨 두도록 설계했다. 그 행을 지우면 §3.8의 결함이 다시 새어나간다.

### 3. `progress_version`이 두 일을 겸한다 — **v2 권위 계약**

> 이동의 낙관적 동시성 토큰이면서, 10초마다 도는 **생존 신호의 카운터**다.
> 후자는 상태를 바꾸지 않는데 같은 숫자를 올린다 (`20260814091000:928`).

실측: `move_count` 1·3인 방의 버전이 **88·86**. 클라이언트는 아무것도 하지 않아도
**분당 6번 뒤처진다.**

C는 클라이언트에서 닫을 수 있는 만큼만 닫았다 — 버전을 **호출 시점에** 읽어
약 15% → 약 1%. 스모크 5회 무충돌이지만 **1%를 반증하지는 않는다** (§3.9).

**반영할 것:** liveness를 토큰에서 분리한다. **비용을 알고 시작해야 한다:**
- 별도 컬럼(예: `heartbeat_seq`)과 **별도 realtime 경로**가 필요하다.
  현재 그 bump가 `room_players` realtime 이벤트를 만들고 **상대 생존 표시**를 굴린다.
- `classifyRealtimeVersion`이 `progress_version`으로 순서를 가리므로
  (`utils/serverAuthority.js`) 소비자가 함께 바뀐다. **`utils/serverAuthority.js`는
  동결 파일이다** — C는 읽기만 했다.
- `apply_*_move_v2`의 `p_expected_version` 비교가 계약이고 **배포된 번들이 부른다.**

**C 범위 밖이라고 판정한 근거가 그것이다** `[사용자 판정, 2026-09-06]`.

### 4. 그룹 경로 `coalesce` — **관찰. 조사 먼저, 수정은 그다음**

`apply_group_move_v2` (`20260814103000:745-798`)는 `resolve_wiki_revision`을 부르고
**`LINK_SNAPSHOT_MISSING` 가드가 없다.** null이 그대로
`coalesce(v_to_revision, current_revision_id)`로 들어가므로, 1:1에서 거부한
**어긋난 `(page_id, revision_id)` 짝이 그룹에서는 표현 가능하다.** 같은 NORMAL_LINK
조회가 그 뒤 모든 클릭을 `LINK_NOT_ALLOWED`로 만든다.

**노출은 좁다:**
- 그룹은 아이템이 강제로 꺼져 있어 (`20260814103000:339` `GROUP_ITEMS_DISABLED`)
  **서버가 목적지를 고르는 경로가 없다** — 1:1을 무너뜨린 그 넓은 노출이 없다.
- NORMAL_LINK는 클라이언트가 목적지를 먼저 스냅샷한다. null이 되려면 그 쓰기가
  실패했거나 사이에 지워졌어야 한다.

**설계상 도달하는 길은 아니다.** 그래서 C는 **확인만 하고 고치지 않았다**
`[사용자 지시, 2026-09-06]` — 동결 영역이고 C 범위 밖이다.

**반영할 것:** 실제로 도달 가능한지 먼저 판정한다. 도달 가능하면 1:1과 같은
형태의 가드가 답이고, **B안식 완화는 여기서도 답이 아니다** (같은 이유).

### 5. `migration 무수정` 연속이 `20260904100000`에서 끊겼다 — 사실 기록

P4~P8이 그 연속을 유지했다. **서버를 건드리지 않았기 때문이고, 유지가 목표였던
적은 없다.** §3.8의 결함은 서버에 있었다 — 후보 쿼리가
`private.apply_duel_move_internal_v3` 안에 있어 **클라이언트 수정으로는 표현할 수
없다.** 그래서 끊었다.
...
| 변경 | 함수 **1개** `create or replace` — 본문은 `20260904090000`의 것과 **추가 필터 12줄을 빼면 바이트 동일** (diff 확인) |
...
**저장소 migration은 15개, 운영은 12개 그대로다** (R6). `20260904100000`은
`20260904090000`과 함께 **아직 운영에 없다** — 운영 반영은 C의 산출이 아니다.

> ⚠ **연속이 끊긴 것을 복구하려 하지 말 것.** 이 문서가 그 연속을 **품질 지표로
> 적은 적이 없다.** P4~P8에서 그것이 참이었던 이유는 서버에 결함이 없어서였고,
> 결함이 서버에 있는 것을 확인한 뒤에는 **클라이언트에서 우회하는 쪽이 더 나쁜
> 선택**이다 — §3.8의 B안이 정확히 그 우회였고, 영구 벽돌로 이어졌다.
