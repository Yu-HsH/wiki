...
작성일: 2026-08-21 · 최종 갱신: **2026-08-28 (창 실행 결과 반영 — §0.-1 신설)**
이전 갱신: 2026-08-27 (§6.0 롤백 판단 기준 신설, §6.3 확정, §6.5 리허설 기록)
기준 커밋: `4a78a0d`
...
`origin/main` 대비: **0 behind / 0 ahead** — W1·W1-a push 후 두 ref가 같은 커밋(`4a78a0d`)이다
(`git ls-remote origin`, 2026-08-28 실측)

**상태: 실행됐다 (2026-08-27~28). W0~W9 수행, W10 미수행, 창 종료.**
결과 요약은 **§0.-1**, 실행 기록 전문은 **`docs/ops/CUTOVER-LOG-2026-08-27.md`** 에 있다.
...
## 0.-1 창 실행 결과 (2026-08-27~28) — **이 문서를 읽기 전에**
...
**이 창이 드러낸 계획 수정 대상 5건** (전문: CUTOVER-LOG §6.2) — 아래 본문에 전부 반영했다:

| # | 사실 | 반영 위치 |
|---|---|---|
| 1 | `VITE_*`는 Vercel **Secret으로 저장할 수 없다.** Type = `Config` | §3.2 **W0** |
| 2 | main push 커밋 수는 **31**이었다 (기재 16) | §3.2 **W1** |
| 3 | **`public` 전용 데이터 덤프가 §4.3 명령 목록에 없어 창에서 빠졌다** | **§4.3** |
| 4 | **`functions deploy`에 `--linked`는 없다** (`Unrecognized flag`) | §3.2 **W8** + 템플릿 |
| 5 | **W6 종료 시각이 기록되지 않았다** — 란은 있었다. 강제가 없었다 | §3.2 **W6** + 템플릿 |

> **다음 창의 범위는 W9 잔여 4건 + W10 하나다.** W0~W8을 다시 하지 않는다 —
> DB·배포 축은 닫혔다. 목록은 `docs/agent/CURRENT.md` §5.5.
...
- ~~창 절차를 **운영에서 실행하지 않았다.**~~ — **2026-08-27~28에 W0~W9를 운영에서 실행했다**
  (§0.-1, `docs/ops/CUTOVER-LOG-2026-08-27.md`). **W10·W11만 이 문서 기준으로 미실행이다.**
...
- **W10만 어떤 형태로도 수행되지 않았다.** W-1·W0·W1·W8·W9는 창에서 실제로 실행됐다.
- 시간 값은 별도 표기가 없으면 여전히 `[추정]`이다. ~~저장소에 **운영 대상** 실행 이력이 없다~~
  ...
  (§0.-1, §3.3.2). §6.5.5의 `[산출물]` 값은 **로컬 실측**이며 운영 값이 아니다 —
