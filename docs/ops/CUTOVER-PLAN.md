...
| P10 | [ ] | **차단 행 측정 + 삭제 범위 확정** | **① §5.3-0의 `w6_blocking_rows`를 측정한다** (읽기 전용). **② §5.3 (1)(2)로 삭제 대상과 CASCADE 영향을 측정한다.** ③ 그 수치를 보고 삭제를 승인한다. **`w6_blocking_rows > 0`이면 삭제를 건너뛰는 선택지는 없다** — W6가 #10에서 `SQLSTATE 23514`로 실패한다 (§6.5.3 리허설 근거). 이 경우 P10은 "삭제할지"가 아니라 **"어디까지 삭제할지"만** 정한다 | `AGENTS.md` §4. **2026-08-27에 성격이 바뀌었다** — 리허설이 F15의 "삭제를 건너뛰어도 migration은 실패하지 않는다"를 반증했다. F15a·§5.3-0·§6.5.3 |
...
- **남은 6건의 성격:** P4·P5·P7·P9는 운영·외부 확인이고 P12는 파일 준비다. **P10은 순수한 사용자 합의가 아니라 운영 측정(`w6_blocking_rows`)이 선행하는 항목이 됐다** — 측정값이 삭제 여부를 강제할 수 있고, 합의 대상은 삭제 범위로 좁혀진다 (§5.3-0).
