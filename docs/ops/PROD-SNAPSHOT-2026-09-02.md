...
| RLS 활성 여부 (`relrowsecurity` 21/21 true) | **실측** |
| **테이블별 정책 수 (합계 33)** | **실측** — 저장소 파생과 **21행 전부 일치**했다 (§4.2) |
...
| 대상 | 왜 없나 |
|---|---|
| **정책 이름·술어** | **수만 실측했다.** §4.2가 보인 대로 `game_rooms`·`room_players`·`room_events`는 **수가 같으면서 내용이 전부 바뀐 자리**이므로, 수 대조는 이름·술어 대조를 대신하지 못한다 |
...
**다섯 축이 움직였고, 전부 `db push --linked`(W6)가 적용한 migration 11개가 만든 것이다.**
**publication만 움직이지 않았다.**
...
| **RLS 정책 합계** | **29** (14테이블) | **33** (21테이블 중 14개가 보유) | W6 — **순증 4건, 내역은 §4.2** |
| migration 이력 | **테이블 자체가 부재** (42P01) | **12행**, 최신 `20260814123000` | W3(`repair`) + W6 |
| publication 멤버십 | 4테이블 | **4테이블 — 변화 없음** | §5 |

> **정책 합계가 +4에 그친 것은 착시일 수 있다.** phase2c가 `game_rooms`·`room_players`·
> `room_events`에서 **7개를 지우고 7개를 다시 만들었기 때문**에 순증에 잡히지 않는다.
> **실제로 손댄 정책은 최소 11개다** (교체 7 + 신설 4). §4.2.
...
| 항목 | 값 |
|---|---|
| `relrowsecurity = true` | **21 / 21** — 전부 활성. RLS 비활성 테이블 **0** |
| 정책 **합계** | **33** |
| 정책 **1개 이상**인 테이블 | **14** |
| RLS 활성 + **정책 0개**인 테이블 | **7** — 신규 5개(`game_mutation_requests`·`group_spectator_emoji_rate_limits`·`wiki_page_snapshots`·`wiki_pages`·`wiki_snapshot_links`) + `picked` + `daily_challenge_pool` |

### 4.2 테이블별 정책 수 — **실측과 저장소 파생이 21/21 일치했다**

**실측값(`[실측]`)과 저장소에서 계산한 기대값(`[코드]`)을 같은 표에 둔다.**
**21행 전부 일치했고 불일치는 0건이다.** 즉 **운영 정책 상태에 미기록 드리프트가 없다.**

파생 계산 방법: baseline의 `CREATE POLICY` 29개에 migration 11개의 `create policy` 9+1+1개를
더하고 `drop policy` 중 **실제로 존재하던 대상만** 뺐다 (`drop ... if exists`가 대부분
no-op이라 단순 가감이 성립하지 않는다).

| 테이블 | 08-20 실측 | **09-02 실측** | 파생 기대값 | 일치 | 변화를 만든 것 |
|---|---:|---:|---:|:---:|---|
| `analytics_events` | 1 | **1** | 1 | ✓ | — |
| `daily_challenge_pool` | 0 | **0** | 0 | ✓ | — |
| `daily_challenges` | 1 | **1** | 1 | ✓ | — |
| `game_move_events` | — | **1** | 1 | ✓ | `20260814090000:281` "Members can read move events" |
| `game_mutation_requests` | — | **0** | 0 | ✓ | — |
| `game_records` | 4 | **4** | 4 | ✓ | — |
| `game_rooms` | 5 | **5** | 5 | ✓ | phase2c가 3개 삭제 후 duel용 3개 재생성 (`:798-800` → `:802·:811·:830`) |
| `group_match_history` | 0 | **1** | 1 | ✓ | phase2c `:783` — RLS 잠금과 함께 조회 정책 신설 |
| `group_match_results` | 3 | **3** | 3 | ✓ | — |
| `group_spectator_emoji_rate_limits` | — | **0** | 0 | ✓ | — |
| `match_history` | 2 | **2** | 2 | ✓ | — |
| `picked` | 0 | **0** | 0 | ✓ | — |
| `profiles` | 5 | **5** | 5 | ✓ | — |
| `room_events` | 2 | **2** | 2 | ✓ | phase2c `:898` 삭제 → `:900` 재생성 |
| `room_players` | 4 | **4** | 4 | ✓ | phase2c `:839-843` 3개 삭제 → `:846·:861·:884` 재생성 |
| `single_game_runs` | — | **1** | 1 | ✓ | `20260814091000:59` "Users can read own single runs" |
| `target_candidates` | 2 | **2** | 2 | ✓ | — |
| `user_profile_stats` | 0 | **1** | 1 | ✓ | phase2c `:791` |
| `wiki_page_snapshots` | — | **0** | 0 | ✓ | — |
| `wiki_pages` | — | **0** | 0 | ✓ | — |
| `wiki_snapshot_links` | — | **0** | 0 | ✓ | — |
| **합계** | **29** | **33** | **33** | **✓** | **+4** |

**정책 합계 29 → 33의 내역 — 순증 4건이고 전부 신설이다** `[코드]`:

| 테이블 | 증감 | 만든 것 |
|---|---:|---|
| `group_match_history` | **+1** | phase2c `:783` — RLS 잠금(`:765`)과 짝을 이룬다 |
| `user_profile_stats` | **+1** | phase2c `:791` — RLS 잠금(`:766`)과 짝을 이룬다 |
| `game_move_events` | **+1** | `20260814090000:281` (새 테이블) |
| `single_game_runs` | **+1** | `20260814091000:59` (새 테이블) |

> **`game_rooms`·`room_players`·`room_events`는 정책이 크게 바뀌었는데 수는 그대로다.**
> phase2c가 이 셋에서 **7개를 지우고 7개를 새로 만들었다** — duel 경로용으로 술어를 바꾼
> 교체였다. **수만 보면 변화가 없어 보이지만 내용은 전부 바뀐 자리다.**
> **정책 수 대조는 이름·술어 대조를 대신하지 못한다** — 그 축은 여전히 미확인이다(§9).

#### 4.2.1 파생 방법이 두 시점에서 검증됐다 — 다음 조사에 쓸 수 있다

**같은 계산 방법이 서로 다른 두 운영 상태를 정확히 재현했다:**

| 시점 | 파생값 | 실측값 | 결과 |
|---|---:|---:|---|
| **2026-08-20** (migration 미적용, baseline만) | 29 / 14테이블 | 29 / 14테이블 | **일치** (08-20 §10.3) |
| **2026-09-02** (migration 11개 적용 후) | **33 / 21테이블** | **33 / 21테이블** | **일치. 21행 전부** |

**함수 수도 같은 방식으로 맞았다** — 7 + 31 − 2 = **36**이고 실측 36이다 (§7.2).
**즉 서로 다른 두 축(정책·함수)에서, 서로 다른 두 시점에, 파생이 실측을 재현했다.**

**따라서 다음 조사에서는 운영 조회 없이 저장소 파생을 1차 근거로 쓸 수 있다.**
`AGENTS.md` §1이 운영 DB 접근을 금지하므로 이것은 실질적인 이득이다 — **"운영을 못 보니
알 수 없다"가 아니라 "저장소에서 계산하고 필요할 때 표본만 대조한다"로 바뀐다.**

**다만 성립 조건이 있다. 조건이 깨지면 파생도 깨진다:**

| 조건 | 왜 필요한가 |
|---|---|
| **운영이 migration 이력과 일치한다** | 이것이 파생의 전제 전부다. **창 이전에는 성립하지 않았다** — 운영 스키마가 CLI 이력 밖에서 구성돼 있었다(§1.1). **W3·W6가 이 조건을 만들었다** |
| **이력 밖 변경이 없다** | 대시보드 SQL Editor로 정책 하나를 바꾸면 **파생은 조용히 틀린다.** 저장소에는 흔적이 남지 않으므로 **파생만으로는 검출할 수 없다** |
| **가감 대상이 명확한 축에만 쓴다** | 정책·함수·테이블처럼 **DDL이 세는 대상**에만 성립한다. **컬럼 기본값·`GRANT`/ACL·인덱스는 이 방법으로 검증되지 않았다**(§9) — 특히 ACL은 `alter default privileges`와 역할 상속이 얽혀 단순 가감이 성립하지 않는다 |

> **그래서 파생은 실측을 대체하지 않고 실측의 빈도를 낮춘다.**
> **운영 DDL을 바꾸는 창을 연 직후에는 반드시 실측으로 재검증한다** — 이번처럼
> 창이 끝난 뒤 한 번 대조해 두면 그 다음부터 파생을 믿을 근거가 생긴다.
...
> **~~테이블별 정책 수 미측정~~ — 닫혔다 (2026-09-02).** 실측 33이 파생 33과 **21행 전부
> 일치했다** (§4.2). 이것이 이 문서의 가장 큰 공백이었다.

| # | 항목 | 성격 |
|---|---|---|
| 1 | **정책 이름·술어 미대조** | 수는 맞았지만 **내용은 대조되지 않았다.** phase2c가 `game_rooms`·`room_players`·`room_events`에서 7개를 교체했고 **수로는 검출되지 않는다** (§4.2) |
...
| 4 | **`GRANT`/ACL 미대조** | migration 11개 적용 **후** 상태는 한 번도 대조되지 않았다. **§4.2.1의 파생으로 대신할 수 없는 축**이다 |
...
**1~3은 §4.2.1의 파생으로 상당 부분 좁힐 수 있다** — 이름·술어·시그니처는 저장소에 전부 있고,
파생이 두 시점에서 검증됐기 때문이다. **4는 다르다.** ACL은 `alter default privileges`와
역할 상속이 얽혀 단순 가감이 성립하지 않으므로 **실측이 필요한 축으로 남는다.**
