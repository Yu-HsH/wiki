...
> #### ⚠ 봉인 — 이 문서는 특정 시점 명세다 (2026-08-29 봉인)
>
> **본문을 현재 사실의 근거로 인용하지 않는다.** 본문은 보존하되 아래 두 축이 이미 낡았다.
> 지금 상태의 단일 기준은 **`docs/agent/CURRENT.md`**, 운영 적용 이력은
> **`docs/ops/CUTOVER-LOG-2026-08-27.md`** 다.
>
> **낡은 축 1 — 시간 규칙.** 본문의 **15분/3분**은 stale이다. 확정값은 **20분/2분**이며
> 기준은 `wiki-race-2.0-handoff/01-CONFIRMED-SPEC.md`다. (이 표기는 이전부터 있었다.)
>
> **낡은 축 2 — 운영 적용 상태. 2026-08-27~28 cutover 창이 뒤집었다.**
>
> | 위치 | 본문 서술 | 실제 (2026-08-28) |
> |---|---|---|
> | 아래 "운영 적용 상태", §4.1 적용 위치 블록 | **미적용** — 로컬에서만 수행 | **적용 완료.** W6에서 migration 11개 전량 적용, W7 전항목 통과 |
> | §5.4 | "다음 테이블은 **현재** RLS 최종 잠금이 **필요**하다" (`group_match_history`, `user_profile_stats`) | **잠금 완료.** Phase 2C(`20260813072952`)가 W6에서 적용돼 **W7 실측 `rls_off_tables = 0` (14/14)** |
> | §13 운영 적용 주의 | "`db push` 금지 / `db reset --linked` 금지 / **`migration repair` 금지**" | **`migration repair`(W3)와 `db push --linked`(W6)는 2026-08-28에 건별 승인 아래 실제로 실행됐다.** 금지 규칙 자체는 `AGENTS.md` §1로 옮겨져 유효하다 — **승인 없이 실행하지 않는다**는 형태이며, 그 창의 승인은 그 창에서 끝났다 |
> | §13 "운영 적용 전 백업·롤백·배포 순서를 별도로 검토" | 미수행 | **수행됐다.** 백업 `CUTOVER-PLAN` §4, 복원 §6.3, 배포 순서 §3.2 (W0→W1→W6) |
>
> **`GROUP_SPECTATOR_MIGRATION.sql`의 "미적용"은 이 창과 무관하다** — 의도적으로 적용하지 않는
> 제안 파일이며 지금도 미적용·미추적 상태다 (`docs/agent/CURRENT.md` §4).
...
> → **작성 시점의 값이다. 2026-08-28 W6로 운영 적용 완료** (위 봉인 헤더 참조).
