...
| **2026-09-04** | **C2** | **`확인 필요` ①②를 확정으로 전환** — ① 50% 감쇠의 정수 처리 = **`floor`** ② 일일/주간 경계 시간대 = **KST(`Asia/Seoul`)**. 남은 `확인 필요`는 **③④⑤ 3건** | **코드 실측.** 트랙 15a(`020daaa` → merge `1af9f93`)가 둘 다 구현했고 **pgTAP 128/128 · `npm test` 252/252**가 값을 고정했다 — `utils/xpRules.js:192`(`Math.floor`) · `:125`(`SERVICE_TIME_ZONE`) · `xp_ledger_v1.sql:372`(25의 50% = 12). **C2 §0** |
