...
| `grace_timeout` | 유예 시간 종료 `확인 필요` — 시안에 문구 없음 |
| `time_limit` | 제한 시간 종료 `확인 필요` |
| `forfeited` · `left` | 없음 |
...
| 상태 | 항목 |
|---|---|
| **확정** | **통일하지 않는다는 방향** · **저장 어휘 7축 전수** · **그룹 4용어 매핑** · **1:1 5경우 매핑과 XP 연결** · 싱글 3경우 · **매핑은 프론트 단일 모듈** · `match_end_reason`은 죽은 컬럼으로 보존 · CHECK 2건의 창 귀속 |
| **확인 필요** | ① `grace_timeout`·`time_limit`의 **부제 문구** (시안에 없다) ② `game_records`에 미완주 기록을 남길 것인가 (§4.1) ③ 매핑 모듈을 `groupResultFormatter.js` 확장으로 할지 신규로 할지 ④ **`cancelled` 결과의 화면 표시** — 확정 스펙 §4.2가 "양쪽 서버 장애는 무효 처리 가능"이라고만 하고 표시를 규정하지 않는다 |

> **①이 남은 이유를 적어 둔다.** 시안은 `몰수 — 재접속 유예 종료` 하나만 부제를 보여준다.
> **나머지 둘의 문구를 발명하지 않았다** — 근거가 없으면 만들지 않는 것이 이 계약의 규칙이다.
