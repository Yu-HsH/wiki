...
## 0. ⚠ 정정 이력 — **`확인 필요` 2건을 확정으로 올렸다**

`C3` §0이 만든 규칙을 따른다 — ① 이 표에 남긴다 ② 본문의 옛 값은 지우지 않고 취소선으로 둔다
③ **근거는 코드 실측이어야 한다.**

| 날짜 | 무엇을 | 어떻게 | 왜 · 근거 `[코드]` |
|---|---|---|---|
| **2026-09-04** | **§8의 `확인 필요` ① — 50% 감쇠의 정수 처리** | ~~`floor` **제안**~~ → **`floor` 확정** | **트랙 15a가 `floor`로 구현했고 양쪽 테스트가 값을 고정했다.** `utils/xpRules.js:192` `Math.floor(safeBase * ratio)` · `supabase/tests/xp_ledger_v1.sql:360-372` `'half of 25 is 12 — floor, not round (C2 §8-①)'` · `tests/xpLedger.test.js:191`. **25 XP의 50%는 13이 아니라 12다.** 반올림이면 base 값에 따라 유불리가 번갈아 나오고 결과 화면에서 설명할 근거가 없다 |
| **2026-09-04** | **§8의 `확인 필요` ② — 일일/주간 경계 시간대** | ~~KST **제안**~~ → **KST(`Asia/Seoul`) 확정** | **이미 배포된 `ensure_today_daily_challenge`가 `now() at time zone 'Asia/Seoul'`로 "오늘"을 정의한다.** 다른 시간대를 쓰면 **"오늘"이 두 뜻이 된다.** 15a가 `utils/xpRules.js:125` `SERVICE_TIME_ZONE = "Asia/Seoul"`로 한 곳에 모았고 `serviceDayKey`/`isSameServiceDay`/`serviceDayStart`가 이 값을 쓴다 · 경계 테스트 `tests/xpLedger.test.js:312-343` |

**근거 커밋: `020daaa` (feat/track-15a).** 통합 시점의 pgTAP **128/128**, `npm test` **252/252** 통과.

**정정의 성격:** 동결된 결정을 **바꾼 것이 아니라 비어 있던 칸을 채운 것이다.**
감쇠 3열 구조(§5)·비율 표(100/50/0%)·`decay_reason` 3값은 그대로다. 바뀐 것은
**"50%를 정수로 어떻게 떨어뜨리는가"와 "하루의 시작이 언제인가"** 두 개뿐이다.

> **주간 경계(§2의 주간 제외 규칙)에도 같은 KST가 적용된다.** ②는 "일일/주간"을 한 항목으로
> 묶고 있었으므로 둘을 갈라 두지 않는다.
...
> ~~**`확인 필요` 2건.**~~ → **둘 다 2026-09-04에 확정되었다.** §0을 본다.
> ① **"50%의 정수 처리"** — `15` §7이 테스트 항목으로 열거하지만 **반올림/내림이 정해지지 않았다.**
> 25 XP의 50%는 12인가 13인가. ~~**`floor` 제안**~~ → **`floor` 확정** (사용자에게 유리한 쪽이 아니라 예측 가능한 쪽).
> ② **일일 경계의 기준 시간대** — `15` §3이 "서비스 기준 시간대를 명확히 사용"이라고만 한다.
> ~~**KST 제안**~~ → **KST 확정** — `ensure_today_daily_challenge`가 이미 `now() at time zone 'Asia/Seoul'`을 쓴다 `[코드]`.
...
| 상태 | 항목 |
|---|---|
| **확정** | 테이블 DDL · **`xp_class` 3종과 주간 제외 규칙** · `source_type` 14종과 XP 값 · 멱등성 3열 유니크 · 감쇠 3열 구조 · RLS · RPC 3개 시그니처 · 0 XP 행 기록 · **① 50% 감쇠의 정수 처리 = `floor`** · **② 일일/주간 경계 시간대 = KST** |
| **확인 필요** | ~~① **50% 감쇠의 정수 처리** (`floor` 제안)~~ **→ 확정, §0** ~~② **일일/주간 경계 시간대** (KST 제안)~~ **→ 확정, §0** ③ **한 완주가 랜덤·오늘 코스에 동시 해당할 때 XP를 겹쳐 주는가** ④ `admin_adjustment`의 `source_id`가 가리킬 운영 기록 테이블이 아직 없다 ⑤ **주간 동점 tie-break** — `15` §5.2가 "현재 데이터로 일관된 tie-break를 문서화"라고만 한다 |

**남은 `확인 필요`는 ③④⑤ 3건이다.** ③은 여전히 스키마에 영향을 준다 (아래 주석).
